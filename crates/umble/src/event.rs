...
/// Event types that are compatible with the umble app loop.
...
pub enum Event {
    ...
    Resumed,
    /// The application is about to wait for a redraw.
    AboutToWait,
}
...
impl WindowEvent {
    ...
    pub fn from_winit_window_event(
        ...
        let event = match event {
            ...
            winit::event::WindowEvent::KeyboardInput { event, .. } => {
                match event.physical_key {
                    PhysicalKey::Code(keycode) => {
                        match event.state {
                            ElementState::Pressed => KeyPressed(keycode),
                            ElementState::Released => KeyReleased(keycode),
                        }
                    }
                    PhysicalKey::Unidentified(_) => {
                        warn!("Unidentified key code");
                        return None;
                    }
                }
            }
            ...
            // move from winit::event::Event to winit::event::WindowEvent
            winit::event::WindowEvent::RedrawRequested { .. } => return None,

            winit::event::WindowEvent::ActivationTokenDone { .. } => return None,
        };
...
impl LoopEvent for Event {
    /// Convert the given `winit::event::Event` to a umble `Event`.
    fn from_winit_event<T>(event: &winit::event::Event<T>, app: &App) -> Option<Self> {
        let event = match event {
            ...
            winit::event::Event::AboutToWait => Event::AboutToWait,
