...
impl wgpu::Texture {
    ...
    pub fn load_from_image(
        ...
        load_texture_from_image(device, queue, usage, image).expect("Failed to load texture.")
    }
    ...
    pub fn encode_load_from_image(
        ...
        encode_load_texture_from_image(device, encoder, usage, image).expect("Unsupported image format.")
    }
...
impl wgpu::RowPaddedBuffer {
    ...
    pub fn from_image_buffer<P, Container>(
        ...
        where
            ...
            Container: Deref<Target=[P::Subpixel]>,
    {
        // TODO - I think this is wrong, but COLOR_TYPE did not exist anymore and i dont know how to fix it in a better way
        let subpixel_size = std::mem::size_of::<P::Subpixel>() as u32;
        let pixel_size = subpixel_size * P::CHANNEL_COUNT as u32;

        let result = Self::new(
            device,
            image_buffer.width() * pixel_size,
...
impl<'buffer> ImageReadMapping<'buffer> {
            ...
            P: Pixel + 'static,
    {
        let subpixel_size = std::mem::size_of::<P::Subpixel>() as u32;
        let pixel_size = subpixel_size * P::CHANNEL_COUNT as u32;
        // TODO - COLOR_TYPE did not exist anymore, comment the assertion below for now
        ...
        let full_image =
            image::ImageBuffer::from_raw(padded_width_pixels, self.buffer.height(), container)
                .expect("umble internal error: incorrect buffer size");
...
impl Pixel for image::Rgb<u16> {
    const TEXTURE_FORMAT: TextureFormat = TextureFormat::Rgba16Uint;
}

impl Pixel for image::Rgb<f32> {
...
pub fn format_from_image_color_type(color_type: image::ColorType) -> Option<wgpu::TextureFormat> {
    let format = match color_type {
        ...
        // TODO: New since 0.19, maybe wrong
...
pub fn load_texture_from_image(
    ...
    usage: wgpu::TextureUsages,
    image: &image::DynamicImage,
) -> Result<wgpu::Texture, ImageFormatError> {
    use image::DynamicImage::*;
    match image {
        ImageLuma8(img) => Ok(load_texture_from_image_buffer(device, queue, usage, img)),
        ImageLumaA8(img) => Ok(load_texture_from_image_buffer(device, queue, usage, img)),
        ImageRgba8(img) => Ok(load_texture_from_image_buffer(device, queue, usage, img)),
        ImageLuma16(img) => Ok(load_texture_from_image_buffer(device, queue, usage, img)),
        ImageLumaA16(img) => Ok(load_texture_from_image_buffer(device, queue, usage, img)),
        ImageRgba16(img) => Ok(load_texture_from_image_buffer(device, queue, usage, img)),
        ImageRgb8(_img) => {
            let img = image.to_rgba8();
            Ok(load_texture_from_image_buffer(device, queue, usage, &img))
        }
        ImageRgb16(_img) => {
        ...
        ImageRgba32F(_img) => {
            let img = image.to_rgba32f();
            Ok(load_texture_from_image_buffer(device, queue, usage, &img))
        }
        _ => Err(ImageFormatError::Unsupported),
    }
}
...
pub fn encode_load_texture_from_image(
    ...
    use image::DynamicImage::*;
    match image {
        ImageLuma8(img) => Ok(encode_load_texture_from_image_buffer(device, encoder, usage, img)),
        ImageLumaA8(img) => Ok(encode_load_texture_from_image_buffer(device, encoder, usage, img)),
        ImageRgba8(img) => Ok(encode_load_texture_from_image_buffer(device, encoder, usage, img)),
        ImageLuma16(img) => Ok(encode_load_texture_from_image_buffer(device, encoder, usage, img)),
        ImageLumaA16(img) => Ok(encode_load_texture_from_image_buffer(device, encoder, usage, img)),
        ImageRgba16(img) => Ok(encode_load_texture_from_image_buffer(device, encoder, usage, img)),
        ImageRgb8(_img) => {
            let img = image.to_rgba8();
            Ok(encode_load_texture_from_image_buffer(device, encoder, usage, &img))
        }
        ImageRgb16(_img) => {
            let img = image.to_rgba16();
            Ok(encode_load_texture_from_image_buffer(device, encoder, usage, &img))
